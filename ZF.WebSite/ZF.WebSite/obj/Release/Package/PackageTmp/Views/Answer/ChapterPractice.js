    ...
    topevery.ajax({
        ...
        if (data.Success) {
            ...
            for (var i = 0; i < row.length; i++) {
                ...
                for (var j = 0; j < row[i].ChapterPracticeOutput.length; j++) {
                    ...
                    html += div +
                        ...
                        '<div style="float: left;font-size: 18px;"  Id="l_' + row[i].ChapterPracticeOutput[j].Id + '"><span></span>' + row[i].ChapterPracticeOutput[j].QuestionContent.replace(/<p>/, "<p>" + (index - 1) + "、") + '</div>' +
                        '<div class="col-xs-12" DataType="' + row[i].ChapterPracticeOutput[j].Type + '"  Id="' + row[i].ChapterPracticeOutput[j].Id + '">' + html2 +
                        '</div>' + div1;
                }
                html3 += ' <p class="tn-ordinal-num" id="" style="background: none;">' + row[i].SubjectName + '：</p>' +
            ...
            $("#tn-ordinal a").bind("click", function () {
                ...
                $('html, body').animate({ scrollTop: ($('#l_' + id).offset().top -60) }, 100);
            });
        }
    });
...
function savePaper() {
    ...
    topevery.ajax({
        ...
        if (data.Result) {
            location.href = "/Answer/ChapterPracticeView?ChapterQuestionsId=" + $("#ChapterQuestionsId").val();
        } else {
            layer.msg("请勿重复提交数据");
        }
    });
