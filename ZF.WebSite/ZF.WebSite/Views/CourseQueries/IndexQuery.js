$(".curSubject").show();
$("#searchcourseinfo").val(topevery.getQueryStringHtlm("query") == null ? "" : decodeURI(topevery.getQueryStringHtlm("query")));
$("#query").val(topevery.getQueryStringHtlm("query") == null ? "" : decodeURI(topevery.getQueryStringHtlm("query")));
var isFree = "";
var sord = "Asc";
...
function CourseInfoAll() {
    console.log(JSON.stringify({ Page: $("#PageIndex").val(), Rows: $("#Rows").val(), IsFree: isFree, Sord: sord, SubjectId: SubjectId, ProjectId: ProjectId, CourseType: CourseType, Type: 0, query: $("#searchcourseinfo").val(), IsValueAdded: 0 }));
    topevery.ajax({
        url: "api/CourseInfo/GetCourseInfoAll",
        data: JSON.stringify({ Page: $("#PageIndex").val(), Rows: $("#Rows").val(), IsFree: isFree, Sord: sord, SubjectId: SubjectId, ProjectId: ProjectId, CourseType: CourseType, Type: 0, query: $("#searchcourseinfo").val(), IsValueAdded: 0 })
    }, function (data) {
        if (data.Success) {
...
$("#AllCourese a").click(function () {
    $('#PageIndex').val("1");
    $('#AllCourese a').removeClass("kclist_hover");
    $(this).addClass("kclist_hover");
