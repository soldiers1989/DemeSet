...
$(function () {
        ...
        topevery.ajax({
            ...
            if (data) {
                if (data.Success) {
                    ...
                    if (courseinfo.ValidityEndDate) {
                        $("#ValidityPeriod").html(topevery.dataTimeView(courseinfo.ValidityEndDate));
                    } 
                    ...
                    ifValueAdded = courseinfo.IsValueAdded;
                    if (courseinfo.Type === 1) {
                        $('#coursecontenttab').text("简介");
                        $('#courseIntroduct').text("概述");
                        var linkCourse = "";
                        ///关联课程链接
                        topevery.ajax({
                            url: 'api/CourseInfo/GetLinkCourse',
                            data: JSON.stringify({ Id: $('#courseid').val() }),
                            async:false
                        }, function (data) {
                            console.log(data);
                            if (data.Success) {
                                ...
                                if (list.length > 0) {
                                    for (var i = 0; i < list.length; i++) {
                                        linkCourse += "<a target=\"_blank\" href='http://zgrskspx.class.com.cn/CourseInfo/CourseInfo?courseId=" + list[i].Id + "&CourseType=0'><<" + list[i].CourseName + ">></a>,";
                                    }
                                    linkCourse = ",请购买" + linkCourse.substr(0, linkCourse.length - 1) + "等课程获取本机考仿真练习";
                                }
                            }
                        })
                        if (topevery.GetCookie("userToken")) {
                            topevery.ajax({
                                ...
                                if (data.Result) {
                                    ...
                                    $(".j-chbtnarea").html("<lable>此机考仿真练习不支持在线购买" + linkCourse + "</lable>");
                                }
                            });
                        } else {
                            $(".j-chbtnarea").html("<lable>此机考仿真练习不支持在线购买" + linkCourse + "</lable>");
                    ...
                    if (courseinfo.IsValueAdded === 1) {
                        $(".j-chbtnarea").html("<lable>此增值服务不支持在线购买</lable>");
                    }
    ...
    function LearnProgress() {
        topevery.ajax({
            ...
            if (data.Result) {
                ...
                if (model.LastWatch) {
                    $('.pTaskLeft').html("上次学习到:<span style=\"padding-right: 20px;font-weight: bold;color: #ff9900;\">" + model.LastWatch + "</span>");
                    $('#btn_start').on('click', function () {
                        if (model.VideoId) {
                            topevery.openClick("/CourseInfo/ChapterVideoPlay?chapterId=\"\"&courseId=" + $('#courseid').val() + "&videoId=" + model.VideoId, 1);
                        }
                    })
                } else {
                    $('#btn_start').on('click', function () {
                ...
                $('#btn_start').on('click', function () {
                    var videoid = "";
                    try {
                        videoid = $($(".j-hovershow")[0]).attr("videoid");
                        if (videoid) {
                            topevery.openClick("/CourseInfo/ChapterVideoPlay?chapterId=\"\"" + "&courseId=" + $('#courseid').val() + "&videoId=" + videoid + "", 1);
                        }
                    } catch (e) {
    ...
    $('#courseappraise').click(function () {
        $('#SubjectBook').attr("style", "display:none;");
        $('#coursearea').attr("style", "display:none;");
        $('#CoursePaper_1').attr("style", "display:none;");
        $('#CoursePaper_2').attr("style", "display:none;");
    ...
    if (topevery.GetCookie("userToken")) {
        ...
        topevery.ajax({
            ...
            if (data.Result) {
                $("#btnLearnNow").unbind();
                $("#btnLearnNow").html("已购买");
                $("#btnLearnNow").show();
                $('#AddToCart,#Unshelve').hide();
                //$("#AddToCart").show();
                GetChapterList(true);
            } else {
    ...
    function GetChapterList(hasBuy) {
        ...
        topevery.ajax({
            ...
            if (data.Success) {
                $('#chapterlist').html("");var contentHtml = "";
                ...
                if (parentChapters.length > 0) {
                    ...
                    for (var i = 0; i < parentChapters.length; i++) {
                        ...
                        if (parentChapters[i].SubChapterList.length > 0) {
                            ...
                            for (var j = 0; j < sublist.length; j++) {
                                ...
                                //若存在视频编号则展示视频内容  state状态 1：已购买 2：
                                var freeShow1 = "";
                                if (sublist[j].VideoId) {
                                    if (sublist[j].VideoLongTime === "0") {
                                        ...
                                        if (hasBuy) {
                                            freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\" state=\"1\" chapterId='" + sublist[j].Id + "'  VideoId='" + sublist[j].VideoId + "'  courseId='" + $('#courseid').val() + "'  chapterName='" + sublist[j].CapterName + "'    target='_blank' >"
                                            ...
                                            if (ifValueAdded == 1) {
                                                freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\"  state=\"7\"  courseId='" + $('#courseid').val() + "'   chapterName=\"" + sublist[j].CapterName + "\" chapterid=\"" + sublist[j].Id + "\"  VideoId='" + sublist[j].VideoId + "'  target='_blank'>"
                                                   + "<span class=\"f-fr\">领取观看</span>"
                                                   + "<div class=\"f-fr ksjicon-look\"></div>"
                                                   + "</a>"
                                                   + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                   + "<span class=\"f-fr f-thide kstime\">"
                                                ...
                                                if (!IsFree) {
                                                    var message = "";
                                                    //判断是否为试听章节
                                                    if (sublist[j].IsTaste == 1 && sublist[j].TasteLongTime != 0) {
                                                        if (!isLogin) {
                                                            if (sublist[j].TasteLongTime2 > 0) {
                                                                state = 3;
                                                                message = "试听" + formatterTasteLongTime(sublist[j].TasteLongTime2);
                                                            } else if (sublist[j].TasteLongTime2 == 0) {
                                                                state = 5;
                                                                message = "登录观看";
                                                            }
                                                        } else {
                                                            message = "试看" + formatterTasteLongTime(sublist[j].TasteLongTime);
                                                            state = 3;
                                                        }
                                                        freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\" state=\"" + state + "\" chapterId='" + sublist[j].Id + "'  courseId='" + $('#courseid').val() + "'   VideoId='" + sublist[j].VideoId + "'chapterName='" + sublist[j].CapterName + "'    target='_blank' >"
                                                            + "<span class=\"f-fr\">" + message + "</span>"
                                                            + "<div class=\"f-fr ksjicon-look\"></div>"
                                                            + "</a>"
                                                            + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                            + "<span class=\"f-fr f-thide kstime\">"
                                                            ...
                                                            + "</span>"
                                                            + "</span>";
                                                    } else if (sublist[j].IsTaste == 1 && sublist[j].TasteLongTime == 0) {
                                                        if (!isLogin) {
                                                            if (sublist[j].TasteLongTime2 > 0) {
                                                                state = 3;
                                                                message = "试听" + formatterTasteLongTime(sublist[j].TasteLongTime2);
                                                        ...
                                                        freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\" state=" + state + "   courseId='" + $('#courseid').val() + "'   chapterName=\"" + sublist[j].CapterName + "\" chapterid=\"" + sublist[j].Id + "\"  VideoId='" + sublist[j].VideoId + "'  target='_blank'>"
                                                            + "<span class=\"f-fr\">" + message + "</span>"
                                                            + "<div class=\"f-fr ksjicon-look\"></div>"
                                                            + "</a>"
                                                            + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                        ...
                                                        freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\"  state=\"5\"  courseId='" + $('#courseid').val() + "'   chapterName=\"" + sublist[j].CapterName + "\" chapterid=\"" + sublist[j].Id + "\"  VideoId='" + sublist[j].VideoId + "'  target='_blank'>"
                                                    ...
                                                    freeShow1 += "<a class=\"f-fr ksjbtn j-hovershow\" state=\"6\" chapterId='" + sublist[j].Id + "'  VideoId='" + sublist[j].VideoId + "'  courseId='" + $('#courseid').val() + "'  chapterName='" + sublist[j].CapterName + "'    target='_blank' >"
                                                        + "<span class=\"f-fr\">免费试听</span>"
                                                        + "<div class=\"f-fr ksjicon-look\"></div>"
                                                        + "</a>"
                                                        + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                        + "<span class=\"f-fr f-thide kstime\">"
                                    ...
                                    if (!sublist[j].VideoUrl && sublist[j].Count > 0) {
                                        html1 = "";
                                        freeShow1 = "<span class=\"f-fr ksinfo\">"
                                                            + "<span class=\"f-fr f-thide \">"
                                                            + "<a class=\"ChapterPractice\" ChapterId=\"" + sublist[j].VideoId + "\"  target='_blank'>在线练习</a>"
                                                            + "</span>"
                                                            + "</span>"
                                                            + "</span>";
                                        getVideoState("2");//在线练习样式
                                    } else if (!sublist[j].VideoUrl && sublist[j].Count == 0) {
                                        freeShow1 = "<span class=\"f-fr ksinfo\">"
                                                            + "<span class=\"f-fr f-thide \">"
                                                            + formatterUnOnline(sublist[j].CapterName)
                                                            + "</span>"
                                                            + "</span>"
                                                            + "</span>";
                                    }
                                ...
                                if (parentChapters[i].SubChapterList[j].SubChapterList) {
                                    ...
                                    for (var k = 0; k < parentChapters[i].SubChapterList[j].SubChapterList.length; k++) {
                                        ...
                                        if (videoList[k].VideoLongTime === "0") {
                                            ...
                                            //判断课程是否购买
                                            if (hasBuy) {
                                                freeShow += "<a class=\"f-fr ksjbtn j-hovershow\" state=\"1\" chapterId='" + videoList[k].Id + "'  VideoId='" + videoList[k].VideoId + "'  courseId='" + $('#courseid').val() + "'    target='_blank' >"
                                                    + "<span class=\"f-fr\">在线观看</span>"
                                                    + "<div class=\"f-fr ksjicon-look\"></div>"
                                                    + "</a>"
                                                    + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                ...
                                                if (ifValueAdded == 1) {
                                                    freeShow += "<a class=\"f-fr ksjbtn j-hovershow\"   state=\"7\"  courseId='" + $('#courseid').val() + "'    chapterid=\"" + videoList[k].Id + "\"  VideoId='" + videoList[k].VideoId + "' target='_blank'>"
                                                        + "<span class=\"f-fr\">领取观看</span>"
                                                        + "<div class=\"f-fr ksjicon-look\"></div>"
                                                        + "</a>"
                                                        + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                    ...
                                                    if (!IsFree) {
                                                        //判断是否为试听章节
                                                        if (videoList[k].IsTaste == 1 && videoList[k].TasteLongTime != 0) {
                                                            if (!isLogin) {
                                                                if (videoList[k].TasteLongTime2 > 0) {
                                                                    state = 3;
                                                                    message = "试听" + formatterTasteLongTime(videoList[k].TasteLongTime2);
                                                                } else if (videoList[k].TasteLongTime2 == 0) {
                                                                    state = 5;
                                                                    message = "登录观看";
                                                                }
                                                            } else {
                                                                message = "试看" + formatterTasteLongTime(videoList[k].TasteLongTime);
                                                                state = 3;
                                                            }

                                                            freeShow += "<a class=\"f-fr ksjbtn j-hovershow\" state=\"" + state + "\" chapterId='" + videoList[k].Id + "'  courseId='" + $('#courseid').val() + "'   VideoId='" + videoList[k].VideoId + "'  target='_blank' >"
                                                                + "<span class=\"f-fr\">" + message + "</span>"
                                                                + "<div class=\"f-fr ksjicon-look\"></div>"
                                                                + "</a>"
                                                                + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                                ...
                                                                + "</span>"
                                                                + "</span>";
                                                        } else if (videoList[k].IsTaste == 1 && videoList[k].TasteLongTime == 0) {
                                                            if (!isLogin) {
                                                                if (videoList[k].TasteLongTime2 > 0) {
                                                                    state = 3;
                                                                    message = "试听" + formatterTasteLongTime(videoList[k].TasteLongTime2);
                                                            ...
                                                            freeShow += "<a class=\"f-fr ksjbtn j-hovershow\"  state=" + state + "   courseId='" + $('#courseid').val() + "'    chapterid=\"" + videoList[k].Id + "\"  VideoId='" + videoList[k].VideoId + "' target='_blank'>"
                                                               + "<span class=\"f-fr\">" + message + "</span>"
                                                               + "<div class=\"f-fr ksjicon-look\"></div>"
                                                               + "</a>"
                                                               + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                            ...
                                                            freeShow += "<a class=\"f-fr ksjbtn j-hovershow\"   state=\"5\"  courseId='" + $('#courseid').val() + "'    chapterid=\"" + videoList[k].Id + "\"  VideoId='" + videoList[k].VideoId + "' target='_blank'>"
                                                                + "<span class=\"f-fr\">购买观看</span>"
                                                                + "<div class=\"f-fr ksjicon-look\"></div>"
                                                                + "</a>"
                                                                + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                                        ...
                                                        freeShow += "<a class=\"f-fr ksjbtn j-hovershow\"  state=\"6\" chapterId='" + videoList[k].Id + "'   courseId='" + $('#courseid').val() + "'  VideoId='" + videoList[k].VideoId + "'    target='_blank' >"
                                                            + "<span class=\"f-fr\">免费试听</span>"
                                                            + "<div class=\"f-fr ksjicon-look\"></div>"
                                                            + "</a>"
                                                            + "<span class=\"f-fr ksinfo j-hoverhide\">"
                                        ...
                                        if (!videoList[k].VideoUrl && videoList[k].Count > 0) {
                                            html11 = "";
                                            freeShow = "<span class=\"f-fr ksinfo\">"
                                                                + "<span class=\"f-fr f-thide \">"
                                                                + "<a class=\"ChapterPractice\" ChapterId=\"" + videoList[k].VideoId + "\"  target='_blank'>在线练习</a>"
                                                                + "</span>"
                                                                + "</span>"
                                                                + "</span>";
                                            getVideoState("2");

                                        } else if (!videoList[k].VideoUrl && videoList[k].Count == 0) {
                                            freeShow = "<span class=\"f-fr ksinfo\">"
                                                                + "<span class=\"f-fr f-thide \">"
                                                                + formatterUnOnline(videoList[k].CapterName)
                                                                + "</span>"
                                                                + "</span>"
                                                                + "</span>";
                                        }
                    ...
                    $(".section").click(function () {
                        if ($(this).find(".j-hovershow").length > 0)
                        {
                            $(this).find(".j-hovershow").click();
                        } else if ($(this).find(".ChapterPractice").length > 0) {
                            $(this).find(".ChapterPractice").click();
                        }
                    }); 

                    //绑定试题练习点击事件
                    $(".ChapterPractice").click(function (e) {
                        ...
                        topevery.ajax({
                            ...
                            if (data.Success) {
                                if (data.Result) {
                                    topevery.openClick("/Answer/ChapterPractice?ChapterId=" + chapterId + "&chapterQuestionsId=" + data.Result.Id + "&PracticeNo=" + data.Result.PracticeNo);
                                } else {
                    ...
                    //绑定视频点击事件
                    $(".j-hovershow").click(function (e) {
                        var chapterId = $(this).attr("chapterId");
                        var videoId = $(this).attr("VideoId");
                        var courseId = $(this).attr("courseId");
                        var state = parseInt($(this).attr('state'));
                        console.log(state);
                        if (state == 1 || state == 6||state==3||state==2) {
                            topevery.openClick("/CourseInfo/ChapterVideoPlay?chapterId=" + chapterId + "&courseId=" + courseId + "&videoId=" + videoId);
                        }else if(state==4){
                            if (topevery.GetCookie("userToken")) {
                                topevery.openClick("/CourseInfo/ChapterVideoPlay?chapterId=" + chapterId + "&courseId=" + courseId + "&videoId=" + videoId);
                            } else {
                            ...
                            parent.layer.msg("请前往学习中心领取增值服务！", {
                                time:4000
                            });
                        }
                        e.stopPropagation();
                    ...
                    $('#CourseChapter').attr('style', "display:none;")
                    $('#CourseChapterId').parent().attr('style', "display:none;");
                    CourseContent();
                }
            }
        });
    ...
    function getVideoState(state) {
        ...
        switch (state) {
            ...
            case "1":
                ...
                break;
            case "2":
                learnStatehtml = "<span class=\"f-fl ksicon ksicon-40\" title=\"在线练习\"></span>";
                break;
            default:
                learnStatehtml = "<span class=\"f-fl ksicon ksicon-0\" title=\"未开始\"></span>"
        }
        ...
        if (data.Success) {
            ...
            if (arr.length > 0) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    resourceContent += "<a onclick=\"downloadResource(this);\" data=\"" + arr[i].ResourceUrl + "\"><li class=\"res " + suffix + "\" data-name=\"" + arr[i].ResourceName + "\" data-link=\"" + arr[i].ResourceUrl + "\"><span >" + arr[i].ResourceName + "</sapn><span>(" + arr[i].ResourceSize + ")</span></li></a>"
                }
                resourceContent += "</ul>"
...
function downloadResource(event) {
    ...
    if (topevery.GetCookie("userToken")) {
        ...
        topevery.ajax({
            ...
            if (data.Result) {
                var url = $(event).attr("data");
                topevery.openClick(url);
                //$(event).attr("href", url).click();
            } else {
                parent.layer.msg("您尚未购买该课程,请购买后再进行操作!");
            }
        });
    } else {
        parent.layer.msg("您尚未登录,请登录后再进行操作!");
    }
}
...
for (var i = 0; i < 2; i++) {
    topevery.ajax({
        url: "api/CourseInfo/GetPaperListWithType?courseId=" + $('#courseid').val() + "&type=" + i,
        data: JSON.stringify({}),
        async: false
    }, function (data) {
        if (data.Success) {
            if (data.Result.length > 0) {
                $('#CoursePaperId' + (i + 1)).parent().attr('style', 'display:Block;');
                $("#CoursePaper_text" + (i + 1)).html(template("CoursePaper_html", data));
                $("#CoursePaper_text" + (i + 1)).find(".ChapterPracticeId").click(function () {
...
//试卷测评
topevery.ajax({
    url: "api/CourseInfo/GetSubjectBook?courseId=" + $('#courseid').val(),
    data: JSON.stringify({})
}, function (data) {
    if (data.Success) {
        if (data.Result.length > 0) {
            $('#SubjectBookId').parent().attr('style', 'display:Block;');
            $("#SubjectBooklist").html(template("SubjectBooklist_html", data));
        }
    }
});
...
function CoursePaper(type) {
    ...
    $('.tabarea >li> a').attr('style', '').removeClass('selected');
    if (type == 0) {
        $('#CoursePaperId1').attr('style', 'color:#ffffff; background:#ff9900; border-radius:4px; padding-left:10px;padding-right:10px;');
        $('#CoursePaper_1').attr("style", "display:block;");
        $('#CoursePaper_2').attr("style", "display:none;");
    } else {
        $('#CoursePaperId2').attr('style', 'color:#ffffff; background:#ff9900; border-radius:4px; padding-left:10px;padding-right:10px;');
        $('#CoursePaper_1').attr("style", "display:none;");
        $('#CoursePaper_2').attr("style", "display:block;");
    }
...
function SubjectBook() {
    parent.setIframeHeight();
    $('#SubjectBook').attr("style", "display:block;");
    $('#coursearea').attr("style", "display:none;");
    $('#appraisecontent').attr("style", "display:none;");
    $('#CoursePaper_1').attr("style", "display:none;");
    $('#CoursePaper_2').attr("style", "display:none;");
    ...
    $('#CourseResource').attr("style", "display:none;");
    $('.tabarea >li> a').attr('style', '').removeClass('selected');
    $('#SubjectBookId').attr('style', 'color:#ffffff; background:#ff9900; border-radius:4px; padding-left:10px;padding-right:10px;');
}
...
function loadLearnPerson() {
    topevery.ajax({
        ...
        //data: JSON.stringify({ IsFree: 1, Page: 1, Rows: 4, SubjectId: topevery.GetCookie("SubjectId"), CourseId: $("#courseid").val() })
        data: JSON.stringify({ IsFree: 1, Page: 1, Rows: 4, CourseId: $("#courseid").val() })
    }, function (data) {
        var arr = data.Result.Rows;
        if (arr && arr.length > 0) {
...
//转换成对应的上线时间
function formatterUnOnline(chaptername) {
    if (chaptername) {
