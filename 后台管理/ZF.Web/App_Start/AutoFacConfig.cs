...
namespace ZF.Web
{
    public class AutoFacConfig
    {
        ...
        public static void Register()
        {
            ...
            Assembly controllerAss = Assembly.Load("ZF.Web");
            ...
            //3.1 加载数据仓储层Topevery.Dapper.Oracle这个程序集。  
            Assembly repositoryAss = Assembly.Load("ZF.Dapper.Db");
            //3.2 反射扫描这个Topevery.Dapper.Oracle.dll程序集中所有的类，得到这个程序集中所有类的集合。  
            Type[] rtypes = repositoryAss.GetTypes().Where(t => t.GetInterfaces().Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IRepository<,>))).ToArray();
            ...
            //3.1 加载数据服务层Topevery.Application这个程序集。  
            Assembly appServerAss = Assembly.Load("ZF.Application");
            //3.2 反射扫描这个Topevery.Application程序集中所有的类，得到这个程序集中所有类的集合。  
            // rtypes = appServerAss.GetTypes().Where(t => typeof(IAppServer<,>).IsAssignableFrom(t)).ToArray();
            rtypes = appServerAss.GetTypes().Where(t => t.GetInterfaces().Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IAppServer<,>))).ToArray();
            //3.3 告诉AutoFac容器，创建rtypes这个集合中所有类的对象实例  
            builder.RegisterTypes(rtypes).AsSelf();
