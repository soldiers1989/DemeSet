...
///初始化
$(function () {
    var addorEditUrl = 'api/SubjectSmallquestion/AddOrEdit';
    //绑定试题类型下拉列表
    topevery.BindSelect("SubjectType", "Common/QuestionTypeRemoveSeven", "", QuestionState);
    //绑定试题状态下拉列表
    ...
    //如果是编辑状态  赋值对应的文本属性
    function Initialize() {
        //if ($("#BigQuestionId").val()) {
        //    topevery.ajax({
        //        url: "api/SubjectBigQuestion/GetOneId",
        //        data: JSON.stringify({ id: $("#BigQuestionId").val() })
        //    }, function (data) {
        //        if (data.Success) {
        ...
        if ($("#Id").val()) {
            topevery.ajax({
                url: "api/SubjectSmallquestion/GetOne",
                data: JSON.stringify({ id: $("#Id").val() })
            }, function (data) {
                if (data.Success) {
                    var row = data.Result;
                    topevery.setParmByLookForm(row, "sumbitForm1");
                    //初始化试题数量以及试题类型
                    ...
                    if (row.VideoId) {
                        $('#VideoId').val(row.VideoId.split(',')).trigger("change");
                    }
                }
            });
        } else {
     ...
     * 绑定验证规则
     */
    $('#sumbitForm1').bootstrapValidatorAndSumbit(addorEditUrl, {
    ...
    //传值特殊文本给后台
    {
        ...
        if (!$("#Id").val()) {
            ...
            topevery.setParmByLookForm({
                ...
                State: $("#State").val(),
                Number: $("#Number").val(),
                BigQuestionId: $("#BigQuestionId").val(),
            }, "sumbitForm1");
            $(':checkbox[type="checkbox"]').each(function () {
                $(':checkbox[type="checkbox"]').removeAttr('checked');
            });
