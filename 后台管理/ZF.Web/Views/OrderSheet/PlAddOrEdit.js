$(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (!result.Success) {
                    ...
                    var row = $.parseJSON(result.Message);
                    for (var i = 0; i < row.length; i++) {
                        if (!(/^1([358][0-9]|4[579]|66|7[0135678]|9[89])[0-9]{8}$/.test(row[i].手机号码))) {
                            layer.alert("校验失败,第" + (i + 2) + "行不是有效的手机号码");
                            return;
                        }
                    }
                    layer.alert("校验成功");
                    $("#RegiestInput").val(result.Message);
    ...
    $("#sumbit").bind("click", function () {
        ...
        var RegiestInput = $("#RegiestInput").val();
        //if (!registerUserId) {
        ...
        topevery.ajax({
            ...
            data: JSON.stringify({
                ...
                remark: remark,
                data: data,
                RegiestInput: $.parseJSON(RegiestInput)
            })
        }, function (data) {
            var message = "提交失败";
            var icon = 1;
