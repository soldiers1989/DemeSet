...
$(function () {
    ...
    topevery.BindSelect("BigClassId", "Common/GetBigClassIdList?Code=zxgl", "--请选择--", Initialize);
    function Initialize() {
        ///初始化From表单以及验证信息
        if ($("#Id").val()) {
            topevery.ajax({
    ...
    $(".return").bindAddSkip("Affiche/Index");
    /**
     * 绑定验证规则
     */
    ...
    {
        Content: 'Content.getContent()', IdFilehiddenFile: '$("input[name=\'IdFilehiddenFile\']").val()'
    }, function () {
        if (!$("#Id").val()) {
            layer.msg("添加成功,可以继续添加下一个资讯！", {
                icon: 1,
                title: false, //不显示标题
                offset: 'auto',
                time: 3000, //10秒后自动关闭
