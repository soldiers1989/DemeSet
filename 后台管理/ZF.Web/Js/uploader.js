...
(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        var webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            server: opts.server ? opts.server : virtualDirName + "File/UploadFile",
                ...
                if (options.callback) {
                    ...
                    switch (file.ext) {
                        ...
                        case "pdf":
                            ...
                            break;
                        case "mp4":
                            file.url = virtualDirName + "Images/mp4.png";
                            break;
                        case "xls":
                        case "xlsx":
                    ...
                    if ($inputHidden.val() === "") {
                        $inputHidden.val(file.name);
                    } else {
                        $inputHidden.val($inputHidden.val() + "," + file.name);
                    }
                    var li = target.next().next().find("ul").clone().html();
                    li = li.replace("{imgUrl}", file.url).replace("{imgName}", file.name).replace("{imgName}", file.name).replace("{href}", response.Message).replace("{href}", response.Message)
                    .replace("{fileId}", file.name);
    ...
    $(".uploadImg").on("click", "a", function () {
        ...
        if ($hiddenFile.val()) {
            ...
            for (var i = 0; i < files.length; i++) {
                if ($(this)[0].id !== "" & files[i].indexOf($(this)[0].id) > -1) {
                    index = i;
                    break;
                }
            }
