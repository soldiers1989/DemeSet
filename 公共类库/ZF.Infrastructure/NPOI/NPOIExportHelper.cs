...
namespace ZF.Infrastructure.NPOI
{
    ...
    public class NpoiExportHelper
    {
        ...
        /// 导出方式 1：WEB导出 2：路径导出  
        /// </summary>  
        public int ExportMode { get; private set; }
        /// <summary>  
        /// 数据源  
        /// </summary>  
        public DataSet DsSource { get; private set; }
        /// <summary>  
        /// 数据库采用字段  
        /// </summary>  
        ...
        public NpoiExportHelper(string fileName, string sheetName, string header, DataSet ds, string filed)
        {
            // 判断文件后缀名是否存在  
            if (string.IsNullOrEmpty(fileName))
                fileName = "新建Excel.xls";
            else if (fileName.IndexOf(".xls", StringComparison.Ordinal) == -1 && fileName.IndexOf(".xlsx", StringComparison.Ordinal) == -1)
                fileName += ".xls";
        ...
        /// <param name="ds">数据来源</param>  
        /// <param name="filed">数据字段</param>  
        public NpoiExportHelper(string fileName, string sheetName, string header, string title, DataSet ds, string filed, string filePath = null, int exportMode = 1)
        {
            // 判断文件后缀名是否存在  
            if (string.IsNullOrEmpty(fileName))
                fileName = "新建Excel.xls";
            else if (fileName.IndexOf(".xls", StringComparison.Ordinal) == -1 && fileName.IndexOf(".xlsx") == -1)
                fileName += ".xls";
        ...
        {
            #region 变量声明  
            string[] tableTitle = this.Header.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
            // 表头数组  
            string[] newHeaders = null;
            // 数据字段  
            string[] files = this.Filed.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
            // 数据字段  
            string[] file = null;
            // 临时数组  
            string[] temp = null;
            ...
            // 工作薄名称  
            string[] sheetNames = this.SheetName.Split(new string[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
            // 表头名称  
            string[] titles = this.Title == null ? null : this.Title.Split(new string[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
            // 获取行数  
            int rows = GetRowCount(this.Header);
            // 列数计数器  
            int cols = 0;
            // 列头跨行数  
            int rowSpans = 0;
            // 列头跨列数  
            int colSpans = GetColCount(this.Header);
            // HSSFSheet 对象  
            HSSFSheet sheet = null;
            // IRow 对象  
            IRow row = null;
            // 表头行添加  
            int trow = (string.IsNullOrEmpty(this.Title) ? 0 : 1);
            DataTable dt;
            #endregion
            ...
            for (int k = 0; k < sheetNames.Length; k++)
            {
                ...
                foreach (DataRow dr in this.DsSource.Tables[k].Rows)
                {

                    var dataRow = sheet.CreateRow(rowIndex);
                    foreach (DataColumn column in this.DsSource.Tables[k].Columns)
                    {
            ...
            #region 数据导出  
            // WEB导出  
            if (1 == this.ExportMode)
            {
                System.Web.HttpContext.Current.Response.ContentType = "application/vnd.ms-excel";
                //设置下载的Excel文件名  
                System.Web.HttpContext.Current.Response.AddHeader("Content-Disposition", string.Format("attachment;filename={0}", this.FileName));
            ...
            {
                using (FileStream fs = System.IO.File.Open(this.FilePath, FileMode.Append))
                {
                    _workbook.Write(fs);
                    _workbook = null;
                    return fs;
